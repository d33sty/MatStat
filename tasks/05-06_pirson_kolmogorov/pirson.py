...
X0 = [
    -2.47779e-11, -2.48390e-11, -2.48732e-11, -2.49078e-11, -2.49546e-11,
    -2.49546e-11, -2.49489e-11, -2.49330e-11, -2.49634e-11, -2.49153e-11,
    -2.48980e-11, -2.48304e-11, -2.50186e-11, -2.50640e-11, -2.49451e-11,
    -2.50279e-11, -2.50096e-11, -2.48821e-11, -2.48304e-11, -2.49670e-11,
    -2.48384e-11, -2.48677e-11, -2.49263e-11, -2.49318e-11, -2.48630e-11,
    -2.49104e-11, -2.49060e-11, -2.49383e-11, -2.48665e-11, -2.49377e-11,
    -2.49369e-11, -2.48982e-11, -2.48058e-11, -2.48948e-11, -2.48858e-11,
    -2.48231e-11, -2.48180e-11, -2.49383e-11, -2.46490e-11, -2.49801e-11,
    -2.50328e-11, -2.49705e-11, -2.49776e-11, -2.50668e-11, -2.49098e-11,
    -2.49125e-11, -2.48439e-11, -2.49503e-11, -2.47525e-11, -2.49157e-11,
    -2.48620e-11, -2.48160e-11, -2.49609e-11, -2.48970e-11, -2.49333e-11,
    -2.48891e-11, -2.48551e-11, -2.49153e-11, -2.49359e-11, -2.48630e-11,
    -2.49491e-11, -2.48939e-11, -2.49306e-11, -2.48795e-11, -2.48337e-11,
    -2.49082e-11, -2.50165e-11, -2.48907e-11, -2.48738e-11, -2.47683e-11,
    -2.48636e-11, -2.49913e-11, -2.49841e-11, -2.49566e-11, -2.50086e-11,
    -2.48406e-11, -2.48687e-11, -2.48367e-11, -2.49279e-11, -2.49750e-11,
    -2.49279e-11, -2.48160e-11, -2.49066e-11, -2.48730e-11, -2.49029e-11,
    -2.49699e-11, -2.48695e-11, -2.50356e-11, -2.49499e-11, -2.49387e-11,
    -2.50189e-11, -2.50008e-11, -2.49157e-11, -2.49147e-11, -2.49418e-11,
    -2.49029e-11, -2.50106e-11, -2.49542e-11, -2.48976e-11, -2.50242e-11,
    -2.49328e-11, -2.48524e-11, -2.48522e-11, -2.49583e-11, -2.49454e-11,
    -2.48852e-11, -2.48964e-11, -2.47999e-11, -2.49754e-11, -2.49571e-11,
    -2.49007e-11, -2.50365e-11, -2.49383e-11, -2.49866e-11, -2.49837e-11
]
...
if chi2_stat < chi2_critical:
    print(f"chi^2 = {chi2_stat:.4f} < chi^2_кр = {chi2_critical:.4f}")
    print("-> нулевая гипотеза о нормальном распределении НЕ ОТВЕРГАЕТСЯ")
else:
    print(f"chi^2 = {chi2_stat:.4f} >= chi^2_кр = {chi2_critical:.4f}")
    print("-> нулевая гипотеза о нормальном распределении ОТВЕРГАЕТСЯ")

if p_value_chi2 > alpha:
    print(f"p-value = {p_value_chi2:.4f} > alpha = {alpha}")
    print("-> нулевая гипотеза о нормальном распределении НЕ ОТВЕРГАЕТСЯ")
else:
    print(f"p-value = {p_value_chi2:.4f} <= alpha = {alpha}")
    print("-> нулевая гипотеза о нормальном распределении ОТВЕРГАЕТСЯ")
...
if chi2_stat_unif < chi2_critical_unif:
    print(f"chi^2 = {chi2_stat_unif:.4f} < chi^2_кр = {chi2_critical_unif:.4f}")
    print("-> нулевая гипотеза о равномерном распределении НЕ ОТВЕРГАЕТСЯ")
else:
    print(f"chi^2 = {chi2_stat_unif:.4f} >= chi^2_кр = {chi2_critical_unif:.4f}")
    print("-> нулевая гипотеза о равномерном распределении ОТВЕРГАЕТСЯ")

if p_value_chi2_unif > alpha:
    print(f"p-value = {p_value_chi2_unif:.4f} > alpha = {alpha}")
    print("-> нулевая гипотеза о равномерном распределении НЕ ОТВЕРГАЕТСЯ")
else:
    print(f"p-value = {p_value_chi2_unif:.4f} <= alpha = {alpha}")
    print("-> нулевая гипотеза о равномерном распределении ОТВЕРГАЕТСЯ")
...
if p_val_unif > alpha:
    print(f"p-value = {p_val_unif:.4f} > alpha = {alpha}")
    print("-> нулевая гипотеза о равномерном распределении НЕ ОТВЕРГАЕТСЯ")
else:
    print(f"p-value = {p_val_unif:.4f} <= alpha = {alpha}")
    print("-> нулевая гипотеза о равномерном распределении ОТВЕРГАЕТСЯ")
...
counts, bins_hist, patches = plt.hist(X, bins=20, density=True, alpha=0.6, 
...
plt.subplot(1, 2, 1)
plt.hist(X, bins=20, edgecolor="black", cumulative=True, density=True,
         alpha=0.6, color='skyblue', label="эмпирическая CDF")
x_cdf = np.linspace(min(X), max(X), 100)
plt.plot(x_cdf, stats.norm.cdf(x_cdf, mean_X, std_X), "r-", 
         linewidth=2, label="теоретическая CDF (нормальное)")
plt.xlabel('ток, А')
...
plt.plot(l_space, k_vals, 'b-', lw=2, 
...
plt.subplot(1, 2, 1)
plt.hist(X, bins=20, edgecolor="black", cumulative=True, density=True,
         alpha=0.6, color='lightgreen', label="эмпирическая CDF")
plt.plot(x_cdf, stats.uniform.cdf(x_cdf, a, b-a), "g-", 
         linewidth=2, label="теоретическая CDF (равномерное)")
plt.xlabel('ток, А')
plt.ylabel('вероятность')
...
print(f"  нормальное распределение: chi^2 = {chi2_stat:.4f} < chi^2_кр = {chi2_critical:.4f} -> " + 
      ("НЕ ОТВЕРГАЕТСЯ" if chi2_stat < chi2_critical else "ОТВЕРГАЕТСЯ"))
print(f"  равномерное распределение: chi^2 = {chi2_stat_unif:.4f} < chi^2_кр = {chi2_critical_unif:.4f} -> " + 
      ("НЕ ОТВЕРГАЕТСЯ" if chi2_stat_unif < chi2_critical_unif else "ОТВЕРГАЕТСЯ"))
print()

print("Критерий Колмогорова-Смирнова:")
print(f"  нормальное распределение: p-value = {p_val:.4f} -> " + 
      ("НЕ ОТВЕРГАЕТСЯ" if p_val > alpha else "ОТВЕРГАЕТСЯ"))
print(f"  равномерное распределение: p-value = {p_val_unif:.4f} -> " + 
      ("НЕ ОТВЕРГАЕТСЯ" if p_val_unif > alpha else "ОТВЕРГАЕТСЯ"))
print()
